...
ls57=read.csv('ReflectanceValues/SurfaceReflectanceData/UtahLakeLS57CalibrationFormat.csv')
...
plotls57=ggplot(ls57format)+ geom_boxplot(aes(x=Lake, y=value,color=variable))+ylab("Reflectance (0-10000)")+
  ggtitle("Landsat 5-7 Surface Reflectance Values by Band and by Lake")+ scale_colour_manual(values = c("blue","green","red","magenta","orange","yellow"),name="Band")
plotls57
###################################################################################################################################
##Make R dataframes for model development and application
ls57$Dataset="Calibration"
dataframe=ls57
...
save(dataframe,file="ReflectanceValues/SurfaceReflectanceData/UtahLakeLS57ModelData.Rda")
