...
#Overall
mod2=glm(FieldValue~.,data=modsubdataframe,family=gaussian(link="log"))
mod3=glm(FieldValue~1,data=modsubdataframe,family=gaussian(link="log"))
modoverall=step(mod2,scope=formula(mod3),trace=0)
predicted=(modoverall$fitted.values)
compareoverall=data.frame(predicted)
compareoverall$actual=modsubdataframe$FieldValue
compareoverall$diff=compareoverall$actual-compareoverall$predicted
#Compute RMSE
rmse=sqrt(mean(compareoverall$diff)^2)
print(nrow(subdataframe))
print(paste("RMSE is: ",rmse))
print(paste("R2 is: ",summary(lm(compareoverall$actual~compareoverall$predicted))$r.squared))

plot(compareoverall$predicted~compareoverall$actual,main=paste("Model Predicted vs. Measured for Time Window:",timewindow),xlab="Actual (Chl, ug/L)",ylab="Predicted (Chl, ug/L)")
abline(lm(compareoverall$predicted~compareoverall$actual), col="black")
